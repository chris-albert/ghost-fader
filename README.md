...
`hardware/kicad/ghost-fader-narrow.kicad_pcb` is the same schematic on a second, independent board shaped for a signal chain: 207 x 38 mm, inputs on the right end, outputs on the left, Teensy USB out of the middle of the rear long edge, so the box sits in-line like a pedal. It is as narrow as two side-by-side jacks allow: the NRJ6HF jacks sit on an 18 mm pitch (34.8 mm) and the Teensy, which lies across the board so its USB reaches the rear edge, is 36.6 mm long, which sets the 38 mm. It needs a 3D-printed box: about 40 x 210 mm inside, a ledge under the long board edges, two jack holes in each end wall on the jack centres (10 and 28 mm from the rear edge of the board, at the NRJ6HF bushing height), a micro-USB slot and a 3 mm LED hole in the rear wall. No mounting holes; the four jack nuts hold the board. Routed, DRC clean against the schematic (the same jack-nose silkscreen warnings as the main board), 0.6 mm power tracks, 0.3 mm signals, ground pour both sides. Gerbers in `hardware/gerbers-narrow/` and `hardware/ghost-fader-narrow-gerbers.zip`; renders in `docs/pcb-narrow/`. The 1590BB board above is unchanged.
