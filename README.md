...
Footprints are assigned for everything except the NRJ6HF jacks; take that one from Neutrik or SnapEDA. The schematic is generated by `hardware/kicad/tools/gen.py`; edit it in KiCad directly from here on, or edit the generator and rerun it (it needs KiCad's stock symbol libraries at the macOS install path).
